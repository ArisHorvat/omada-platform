...
export function useTasksApi(params: UseTasksApiParams = {}): UseTasksApiResult {
  ...
  const tasksQuery = useQuery({
    ...
    queryKey: QUERY_KEYS.tasks.paginated(orgId ?? '', page, pageSize, groupId, offeringId),

    queryFn: async () => {

      const data = await unwrap(tasksApi.getAll(page, pageSize, groupId, offeringId));
  ...
  const taskQuery = useQuery({
    ...
    queryFn: async () => unwrap(tasksApi.getById(taskId!)),
  ...
  const createTask = useMutation({
    ...
    mutationFn: async (request: CreateTaskRequest) => unwrap(tasksApi.create(request)),
  ...
  const updateTask = useMutation({
    ...
    mutationFn: async ({ id, request }: { id: string; request: UpdateTaskRequest }) =>
      ...
      unwrap(tasksApi.update(id, request)),
  ...
  const deleteTask = useMutation({
    ...
    mutationFn: (id: string) => unwrap(tasksApi.delete(id)),
  ...
  const toggleTaskCompletion = useMutation({
    ...
    mutationFn: async (task: TaskItemDto) => {
      ...
      return unwrap(tasksApi.update(task.id, request));
